...
class StrategyEditor(tk.Frame):
    ...
    def _show_popup(self, b_index: int):
        ...
        self._popup_window.config(bg=BG_COLOUR)
        ...
        for param in self._extra_params[strat_selected]:
            ...
            temp_label = tk.Label(self._popup_window, bg=BG_COLOUR, fg=FG_COLOUR, text=param['name'], font=BOLD_FONT)
            temp_label.grid(row=row_nb, column=0)

            if param['widget'] == tk.Entry:
                self._extra_input[code_name] = tk.Entry(self._popup_window, bg=BG_COLOUR_2, justify=tk.CENTER, fg=FG_COLOUR, insertbackground=FG_COLOUR)
        ...
        # Validate button
        validation_button = tk.Button(self._popup_window, text='Validate', bg=BG_COLOUR_2, fg=FG_COLOUR, command=lambda: self._validate_parameters(b_index))
    ...
    def _switch_strategy(self, b_index: int):
        for param in ['balance_pct', 'take_profit', 'stop_loss']:
            if self.body_widgets[param][b_index].get() == '':
                self.root.logging_frame.add_log(f'Missing {param} parameters.')
                return
