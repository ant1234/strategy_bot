import logging
import requests
import pprint

logger = logging.getLogger()

# "https://fapi.binance.com"
# "https://testnet.binancefuture.com"

class BinanceFuturesClient:
    def __init__(self, testnet):
        if testnet:
            base_url = 'https://testnet.binancefuture.com'
        else: 
            base_url = 'https://fapi.binance.com'

        self.prices = dict()

        logger.info('Binance futures client successfully initialized.')

    def make_request(self, method, endpoint, data):
        # Make a request to the binance api
        if method == 'GET':
            response = requests.get(self.base_url + endpoint + data)
        else:
            raise ValueError()
        
        if response.status_code == "200":
            return response.json()
        else:
            logger.error('Error while making requests %s to %s : %s (error code: %s)', 
                         method, endpoint, response.json(), response.status_code)
            return None
        

    #  Get a dictionary of all the contracts and their data.
    def get_client(self):

        contracts = dict()

        exchange_info = self.make_request('GET', '/fapi/v1/exchangeInfo', None)

        if exchange_info is not None:
            for contract_data in exchange_info['symbols']:
                contracts[contract_data['pairs']] = contract_data    
        
        return contracts
    

    def get_historical_candles(self, symbol, interval):
        data = dict()
        data['symbol'] = symbol
        data['interval'] = interval
        data['limit'] = 1000

        raw_candles = self.make_request('GET', '/fapi/v1/klines', data)

        candles = []

        if raw_candles is not None:
            for c in raw_candles:
                candles.append(c[0], float(c[1]), float(c[2]), float(c[3]), float(c[4]), float(c[5]))
                
        return candles
    
    def get_bid_ask(self, symbol):

        data = dict()
        data['symbol'] = symbol
        ob_data = self.make_request('GET', '/fapi/v1/ticker/bookTicker', data)

        if ob_data is not None:
            if symbol not in self.prices:
                self.prices[symbol] = {
                    'bid': float(ob_data['bidPrice']), 
                    'ask': float(ob_data['ask']) 
                    }
            else: 
                self.prices[symbol]['bid'] = float(ob_data['bidPrice'])
                self.prices[symbol]['ask'] = float(ob_data['askPrice'])

        return self.prices[symbol]
        
